# IES-013 — Consumer Implementation Plan

**Standard:** IES-013 — Consumer Sector Engine
**Milestone:** IES-013.2 (gated work packages)
**Status:** PLAN — execution roadmap (not code)
**Date:** 2026-08-08
**Baseline:** IES-005 + IES-005.1 (contracts) + seven released engines v1.0 + **IES-013 v1.0 (frozen, tag `ies-013-v1.0.0`)** + CSIP (ontology registration, zero change)
**Reuse:** `iips-platform` runtime/framework (validated by eight sectors incl. CSIP — no platform change)
...
- Produce `CONSUMER_IMPLEMENTATION_REUSE_REPORT.md` at completion.
- Reference the Consumer Implementation Traceability Register.

---

## WP-1 — Platform Reuse Verification (evidence gathering)

**Objective:** Confirm `sector.consumer` registers/executes via existing runtime with **zero platform changes**.
...
**Deliverables:** Consumer plugin skeleton registered/executed; reuse evidence.
**Acceptance:** engine hosts with zero platform changes; prior 8-plugin regression unchanged; `tsc --strict` clean.
**Risk:** Low (validated 8×).
...
| 2.5 Coexistence | 7 sectors + CSIP + Consumer |

**Deliverables:** engine registered through all framework services; coexistence regression.
**Acceptance:** same services serve all nine plugins without branching; `tsc --strict` clean; tests pass.
**Risk:** Low.

---

## WP-3 — Consumer Engine (sector-specific)

**Objective:** Implement Consumer logic using **frozen assets only**.

| Task | Notes |
|---|---|
| 3.1 Metric evaluation | CM-001..CM-012 (frozen) |
| 3.2 Score calculation | band→score→pillar→composite (D15) |
| 3.3 Segment + business-model calibration | `consumer-calibration-1.0.0` |
| 3.4 Decision pipeline + overrides + precedence | D10 + Override Precedence |
| 3.5 Evidence generation | consumer evidence packages |
| 3.6 Ontology registration | 8-dimension metadata for CSIP (zero CSIP change) |

**Deliverables:** Consumer Engine plugin (`sector.consumer`) reproducing frozen outputs.
**Acceptance:** **Golden Dataset Reproducibility (10/10)**, **Replay Determinism**, **Calibration Isolation**, **Evidence Completeness**, **Override Precedence**, **Ontology Registration**. `tsc --strict` clean; tests pass.
...
| Task | Notes |
|---|---|
| 4.1 Golden dataset regression | vs `consumer-expected-outputs-1.0.0` (10/10) |
...
| 4.4 Traceability + reuse report | `CONSUMER_ENGINE_ACCEPTANCE_MATRIX.md` + reuse report |
...
**Acceptance:** 100% match vs frozen outputs; replay byte-identical; all 10 fixtures; independent clean-clone reproduction; release candidate.
**Risk:** Low–Medium.
