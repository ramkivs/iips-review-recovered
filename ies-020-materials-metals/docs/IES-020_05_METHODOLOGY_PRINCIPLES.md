# IES-020 AUTHORITY REVIEW.md 05 METHODOLOGY PRINCIPLES
...
# Document 05 - METHODOLOGY PRINCIPLES (IES-020 remediated content)
...
- §0 data authority row (HISTORICAL): > \| Data authority \| **PENDING** — all fixtures PROPOSED synthetic (IES-015/016/017 convention) \|
- §27 certification-data (HISTORICAL): > After methodology authority: freeze the calibration, golden-reference, validation-fixtures, expected-outputs, replay-dataset, and ontology metadata with recorded SHA256; acceptance matrix + readiness certificate + risk register + release notes. All currently **PROPOSED/ILLUSTRATIVE — NOT FROZEN**.
- Terminal status line (HISTORICAL): > **IES-020 D20 SPECIFICATION STATUS: METHODOLOGY ACCEPTED (M1–M15 + G1–G6, recorded 2026-08-20); CERTIFICATION DATA FROZEN**

## 3. Current authority reconciliation

- Methodology: `D20 v1.0` = ACCEPT (DEC-D25 §1). DEC-D25 §7.3 addresses the five open questions without resolving them (Q1 not resolved by that gate; not necessary for the acceptance determination).
...
| --- | --- | --- |
| DEC-D25 acceptance (2026-08-29) | blob cbab4da9ce922aacf45e513954d6e325bb037810 | methodology D20 v1.0 = ACCEPT (fresh, forward-looking; A2 preserved) |
| FROZEN manifest | FROZEN, 2026-08-29, methodologyVersion "IES-020 v1.0 (D20 normative contract)", calibrationProfile materials-metals-calibration-1.0.0 | certification data frozen; postFreezeRule binds |
| Acceptance matrix (16 gates PASS) | ies-020-materials-metals/MATERIALS_METALS_ENGINE_ACCEPTANCE_MATRIX.md | implementation conformance recorded |
| Readiness certificate (recorded) | ies-020-materials-metals/IES-020_IMPLEMENTATION_READINESS_CERTIFICATE.md | evidence-report record incl. the recorded confidence decision |
| Historical pack self-label being reconciled | ies-020-materials-metals/MATERIALS_METALS_DISCOVERY_PACK.md | quoted in §2 above; superseded for authority purposes, retained as history |

## 4. Methodology principles (as accepted; unchanged by this remediation - W8/W9)

1. **Determinism.** No random, clock, or external data at execution time; identical input -> identical output/evidence/metadata (pack §20-21 (M11/M15)).
2. **Band semantics.** Metric -> band -> score; bands are lower-inclusive / upper-exclusive, terminal band includes its upper bound (pack §13-15 (M10)).
3. **Effective band-table resolution.** effective table = calibrated ?? baseline, boundaries AND scores together, only at equal band cardinality (pack §13-15 (M10)).
4. **Missing-data honesty.** A missing primitive is dropped and the pillar renormalizes over available metrics; an empty pillar is 0.0; nothing is fabricated, never NaN (pack §6-8 (M1/M2/M3; MM-001..MM-012); proven by MM-014).
5. **Pillar composition and composite.** Weighted pillars per pack §9-12 (M6/M7); composite = round-half-to-even at 1 decimal (composite only), left-to-right IEEE-754 summation in the fixed pillar order (pack §20-21 (M11/M15); raw 63.25 rounds to 63.2 (ties-to-even)).
6. **Verdict mapping and overrides.** Six-value verdict vocabulary (pack §17-19 (M12)); overrides apply deterministic min-rank precedence, governance -> Avoid, all other caps -> Watch; leverage-breach fires automatically at `debtEbitda >= leverageAlert` (pack §17-19 (M13/M14)).
...
8. **No fabrication / synthetic provenance.** The 13 reference providers are synthetic deterministic fixtures (IES-015 convention), never presented as real companies (pack §27).
...
| # | Question (pack §28, verbatim subject) | Recorded partial disposition (no answer invented) |
| --- | --- | --- |
| Q1 | Aluminium placement (`base-metals` vs `specialty-materials`) | OPEN - proposal: primary aluminium under base-metals, rolled/extruded under specialty-materials; DEC-D25 §7.3 did not resolve it |
| Q2 | `cashCostCurve` percentile as input primitive vs absolute unit cash cost | OPEN - implemented contract uses the percentile position ("Cost-curve percentile position", DEC-D25 §4) |
| Q3 | `reserveLife` commodity-weighting | OPEN - implemented contract uses unweighted "Proven+probable reserves / annual production" (DEC-D25 §4) |
| Q4 | Confidence treatment (G-decision - Option-A analog or other) | OPEN at pack level (§16/§28); the frozen readiness certificate separately records a G5 Option-A analog decision - both records carried, no D7-programme resolution asserted |
| Q5 | Whether `royalty`/`streaming` is a legitimate archetype or should be excluded | OPEN - implemented contract admits `royalty` (risk 0.8) and uses it in the golden reference; `streaming` absent (DEC-D25 §4) |

DEC-D25 §4 records the implemented-contract partial dispositions verbatim: "Q2 `cashCostCurve` = *"Cost-curve percentile position"*; Q3 `reserveLife` = *"Proven+probable reserves ÷ annual production"* (no commodity weighting); Q5 `royalty` **admitted** (risk 0.8) and used in the golden reference, `streaming` **absent**"
...
| Pack self-labels (historical wording) | ies-020-materials-metals/MATERIALS_METALS_DISCOVERY_PACK.md | 7677ec47a335d0157411830a80aba29912dc97b5 |
| Freeze record | ies-020-materials-metals/IES-020_FREEZE_MANIFEST.json | 0d43a538734c9c13645778b0eadfbd978730f637 |
| Normative contract sections | ies-020-materials-metals/MATERIALS_METALS_DISCOVERY_PACK.md | 7677ec47a335d0157411830a80aba29912dc97b5 |
...
- IES-020 pack §28 Q1-Q5 remain OPEN with recorded partial dispositions only (table in §5 of this document); no answer is invented.
