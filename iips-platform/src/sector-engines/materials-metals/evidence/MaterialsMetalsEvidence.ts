/**
 * Materials & Metals Evidence (D20) — evidence packages per D11. Reuses EvidencePipeline.
 *
...
import type { MaterialsMetalsDecisionResult } from '../decision/MaterialsMetalsDecision';
import type { MaterialsMetalsScoreResult } from '../scoring/MaterialsMetalsScoreEngine';

export interface MaterialsMetalsEvidenceInput {
  subsegment: string;
  archetype: string;
  decision: MaterialsMetalsDecisionResult;
  score: MaterialsMetalsScoreResult;
  calibrationVersion: string;
  snapshotId: string;
}

export class MaterialsMetalsEvidence {
  constructor(private readonly pipeline: EvidencePipeline) {}

  build(input: MaterialsMetalsEvidenceInput): Readonly<EvidencePackage> {
    return this.pipeline.build({
      ...
      provenance: {
        ...
        engineVersion: '1.0.0',
        methodologyVersion: 'IES-020 v1.0',
