/** Capital Markets Sector Engine (WP-3) — implements SectorPlugin, consumes platform services, frozen assets. */
import type { SectorPlugin, ExecutionRequest, ExecutionResult, PluginManifest } from '../../plugin-loader/PluginContract';
import type { Container } from '../../di/Container';
import type { RuntimeCoordinator } from '../../runtime/RuntimeCoordinator';
import type { EvidencePipeline } from '../../framework/evidence/EvidencePipeline';
import { CapitalMarketsMetrics, type CapitalMarketsInput } from './metrics/CapitalMarketsMetrics';
import { CapitalMarketsScoreEngine } from './scoring/CapitalMarketsScoreEngine';
import { loadCapitalMarketsCalibration, type CapitalMarketsCalibrationProfile } from './calibration/CapitalMarketsCalibration';
import { CapitalMarketsDecision } from './decision/CapitalMarketsDecision';
import { CapitalMarketsEvidence } from './evidence/CapitalMarketsEvidence';

export const CAPITAL_MARKETS_ENGINE_ID = 'sector.capital-markets';

export class CapitalMarketsEngine implements SectorPlugin {
  readonly identity = {
    engineId: CAPITAL_MARKETS_ENGINE_ID,
    sectorFamily: 'Capital Markets',
    engineVersion: '1.0.0',
    secVersion: '1.0',
    semcVersion: '1.0',
  };
  readonly manifest: PluginManifest = {
    engineId: CAPITAL_MARKETS_ENGINE_ID,
    sectorFamily: 'Capital Markets',
    engineVersion: '1.0.0',
    capabilities: ['metrics', 'scoring', 'calibration', 'decision', 'evidence'],
    compatibility: { framework: '1.0', methodology: 'IES-008 v1.0' },
  };

  private calibration: CapitalMarketsCalibrationProfile = loadCapitalMarketsCalibration();
  private metrics = new CapitalMarketsMetrics();
  private scoreEngine = new CapitalMarketsScoreEngine();
  private decision!: CapitalMarketsDecision;
  private evidence!: CapitalMarketsEvidence;
  private runtime!: RuntimeCoordinator;
  private evidencePipeline!: EvidencePipeline;

  onDiscover(): void { /* no-op */ }

  onRegister(_ctx: Container): boolean {
    this.decision = new CapitalMarketsDecision(this.calibration);
    return true;
  }

  onInitialize(ctx: Container): void {
    this.evidencePipeline = ctx.resolve<EvidencePipeline>('evidenceService');
    this.runtime = ctx.resolve<RuntimeCoordinator>('runtimeCoordinator');
    this.evidence = new CapitalMarketsEvidence(this.evidencePipeline);
  }

  execute(_ctx: Container, request: ExecutionRequest): ExecutionResult {
    const input = request.inputs as unknown as CapitalMarketsInput;
    const metrics = this.metrics.evaluate(input);
    const score = this.scoreEngine.score(metrics);
    const decision = this.decision.decide({
      composite: score.composite,
      costToIncome: metrics['CM-004'],
      aumGrowth: metrics['CM-002'],
      regulatoryFlag: input.regulatoryFlag as string | undefined,
      governanceFlag: input.governanceFlag as string | undefined,
      marketCycleFlag: input.marketCycleFlag as string | undefined,
      confidence: 0.8,
    });

    const snapshot = this.runtime.recordSnapshot(
      CAPITAL_MARKETS_ENGINE_ID,
      metrics,
      score.pillars as unknown as Record<string, number>,
      decision.verdict,
    );
    const evidence = this.evidence.build({
      engineId: CAPITAL_MARKETS_ENGINE_ID,
      metrics,
      score,
      decision,
      calibrationVersion: this.calibration.version,
      snapshotId: snapshot.snapshotId,
      frameworkVersion: '1.0',
      methodologyVersion: 'IES-008 v1.0',
    });

    return {
      state: 'COMPLETED',
      snapshotRef: snapshot.snapshotId,
      evidenceRef: evidence.evidenceId,
      metadata: { verdict: decision.verdict, composite: decision.composite, evidenceId: evidence.evidenceId },
    };
  }

  onComplete(): void { /* no-op */ }
}
