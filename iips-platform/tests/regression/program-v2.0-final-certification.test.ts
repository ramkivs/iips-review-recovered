...
test('FC-CORE: END-TO-END CONSTITUTIONAL EQUIVALENCE — v1.1 path (A) == v2.0 path (B) across all 10 sectors', () => {
...
test('FC-10: cross-sector — all 10 engines retain v1.1 semantics', () => {
  for (const s of BASELINE.sectors) {
    const A = v11Result(s.engineId, ENGINE_FACTORY[s.engineId], s.input);
