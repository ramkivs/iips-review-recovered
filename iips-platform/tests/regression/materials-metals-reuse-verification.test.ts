/**
 * IES-020 — Platform Reuse Verification (D28 fence-4 relief, Tier-3 A1 evidence).
 * Proves the existing platform hosts Materials & Metals (sector.materials-metals) with zero platform changes,
...
test('IES020-RV-ACC3: 14 plugins coexist (12 peers + CSIP + materials-metals)', () => {
...
test('IES020-RV-ACC4: materials-metals replay-compatible deterministic execution via shared services', () => {
  const run = () => {
    const { plugins, runtime } = makeRuntime();
    plugins.load(new MaterialsMetalsEngine());
    plugins.initialize(MATERIALS_METALS_ENGINE_ID);
    return runtime.execute(MATERIALS_METALS_ENGINE_ID, { requestId: 'ies020-replay', inputs: {"subsegment":"base-metals","archetype":"pure-play","ebitdaMargin":30,"revenueGrowth":10,"debtEbitda":1.6,"reserveLife":14,"cashCostCurve":30,"realizedPriceSpread":101,"fcfYield":6,"roic":13,"capexIntensity":14,"inventoryDays":45,"evEbitda":6,"recyclingInputMix":10} });
  };
  const a = run();
  const b = run();
