/**
 * IES-016 — Platform Reuse Verification (D28 fence-4 relief, Tier-3 A1 evidence).
 * Proves the existing platform hosts Telecommunications (sector.telecommunications) with zero platform changes,
...
test('IES016-RV-ACC1: sector.telecommunications registers + executes through the existing runtime', () => {
  const { plugins, runtime } = makeRuntime();
  assert.equal(plugins.load(new TelecommunicationsEngine()), true);
  plugins.initialize(TELECOMMUNICATIONS_ENGINE_ID);
  const r = runtime.execute(TELECOMMUNICATIONS_ENGINE_ID, { requestId: 'ies016-r1', inputs: {"subsegment":"wireless-mno","archetype":"consumer","ebitdaMargin":42,"revenueGrowth":6,"debtEbitda":2.2,"arpu":34,"churnRate":1.1,"postpaidMix":88,"fcfYield":6,"roic":12,"capexIntensity":15,"spectrumCost":0.8,"evEbitda":6.5,"usageGrowth":22} });
  assert.equal(r.result.state, 'COMPLETED');
  assert.equal(r.result.metadata.composite, 77.8);
  assert.equal(r.result.metadata.verdict, 'Buy');
});

test('IES016-RV-ACC2: telecommunications produces snapshots + replays via shared services', () => {
...
test('IES016-RV-ACC3: 14 plugins coexist (12 peers + CSIP + telecommunications)', () => {
...
test('IES016-RV-ACC4: telecommunications replay-compatible deterministic execution via shared services', () => {
  const run = () => {
    const { plugins, runtime } = makeRuntime();
    plugins.load(new TelecommunicationsEngine());
    plugins.initialize(TELECOMMUNICATIONS_ENGINE_ID);
    return runtime.execute(TELECOMMUNICATIONS_ENGINE_ID, { requestId: 'ies016-replay', inputs: {"subsegment":"fixed-broadband","archetype":"enterprise","ebitdaMargin":38,"revenueGrowth":4,"debtEbitda":2.5,"arpu":45,"churnRate":1.4,"postpaidMix":75,"fcfYield":5,"roic":11,"capexIntensity":12,"spectrumCost":0.3,"evEbitda":7,"usageGrowth":14} });
  };
  const a = run();
  const b = run();
