/**
 * IES-013 WP-1 — Platform Reuse Verification.
 * Proves the existing platform hosts a ninth plugin (sector.consumer) with zero
 * platform changes, coexists with the seven sector engines + CSIP, and produces
...
test('IES013-WP1-ACC1: sector.consumer registers + executes through the existing runtime', () => {
  const { plugins, runtime } = makeRuntime();
  assert.equal(plugins.load(new ConsumerEngine()), true);
  plugins.initialize(CONSUMER_ENGINE_ID);
  const r = runtime.execute(CONSUMER_ENGINE_ID, {
    requestId: 'cs-r1',
    inputs: { segment: 'staples', businessModel: 'branded', revenueGrowth: 4, priceContribution: 65, brandLoyalty: 85, marginResilience: 0.9, dtcShare: 20, fcfYield: 6, innovationIntensity: 10, privateLabelExposure: 10, ebitdaMargin: 22, debtEbitda: 2.2, peRatio: 20, roic: 16 },
  });
  assert.equal(r.result.state, 'COMPLETED');
  assert.ok(r.result.metadata.composite !== undefined);
});

test('IES013-WP1-ACC2: consumer produces snapshots + replays via shared services', () => {
...
test('IES013-WP1-ACC3: NINE plugins coexist (7 sectors + CSIP + Consumer)', () => {
...
test('IES013-WP1-ACC4: consumer ontology metadata covers 8 dimensions (CSIP-compatible)', () => {
