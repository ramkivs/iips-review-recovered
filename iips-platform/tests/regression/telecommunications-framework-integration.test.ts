/**
 * IES-016 — Framework Integration (D28 fence-4 relief, Tier-3 A1 evidence).
 * Proves Telecommunications integrates through all shared framework services unchanged (manifest,
 ...
 * Inputs are the frozen golden-reference providers TC-001 / TC-002.
 */
...
test('IES016-FI-ACC1: telecommunications manifest via shared ManifestLoader', () => {
  const engine = new TelecommunicationsEngine();
  const m = new ManifestLoader().load(engine.manifest);
  assert.equal(m.engineId, TELECOMMUNICATIONS_ENGINE_ID);
  assert.equal(Object.isFrozen(m), true);
  assert.ok(m.capabilities.includes('calibration'));
});

test('IES016-FI-ACC2: telecommunications evidence via shared EvidencePipeline', () => {
  const { evidence } = makeRuntime();
  const pkg = evidence.build({
    engineId: TELECOMMUNICATIONS_ENGINE_ID, recommendation: 'Buy', compositeScore: 77.8, confidence: 0.8,
    supportingScores: [{ id: 'arpu', name: 'ARPU', value: 34 }],
    calibrationVersion: 'telecommunications-calibration-1.0.0', replayReference: 'snap',
    provenance: { frameworkVersion: '1.0', engineVersion: '1.0.0', methodologyVersion: 'IES-016 v1.0', snapshotId: 's1' },
  });
  assert.equal(evidence.validate(pkg), true);
});

test('IES016-FI-ACC3: telecommunications deterministic snapshot via shared Snapshot', () => {
  ...
  const r = runtime.execute(TELECOMMUNICATIONS_ENGINE_ID, { requestId: 'ies016-snap', inputs: {"subsegment":"wireless-mno","archetype":"consumer","ebitdaMargin":42,"revenueGrowth":6,"debtEbitda":2.2,"arpu":34,"churnRate":1.1,"postpaidMix":88,"fcfYield":6,"roic":12,"capexIntensity":15,"spectrumCost":0.8,"evEbitda":6.5,"usageGrowth":22} });
  assert.equal(r.result.state, 'COMPLETED');
  assert.equal(store.size, 1);
  assert.ok(r.result.snapshotRef);
});

test('IES016-FI-ACC4: telecommunications replay via shared ReplayService', () => {
  const { plugins, runtime, replay } = makeRuntime();
  plugins.load(new TelecommunicationsEngine());
  plugins.initialize(TELECOMMUNICATIONS_ENGINE_ID);
  const snap = runtime.recordSnapshot(TELECOMMUNICATIONS_ENGINE_ID, { revenueGrowth: 6 }, { composite: 77.8 }, 'TELECOMMUNICATIONS_SUMMARY');
  assert.equal(Object.isFrozen(snap), true);
  assert.equal(replay.replay(snap.snapshotId)?.reproduced, true);
});

test('IES016-FI-ACC5: telecommunications diagnostics + qualification + activation via shared framework', () => {
  const d = new DiagnosticsService();
  const q = new QualificationService();
  const a = new ActivationService();
  d.capture({ engineId: TELECOMMUNICATIONS_ENGINE_ID, executionDurationMs: 3, registryVersions: { transport: '1.0' }, replayStatus: 'ok', transportStatus: 'ok', pluginPhase: 'Execution' });
  assert.equal(d.list()[0].engineId, TELECOMMUNICATIONS_ENGINE_ID);
  const qual = q.qualify({ engineId: TELECOMMUNICATIONS_ENGINE_ID, certified: true, replayVerified: true, regressionPassed: true, deterministic: true });
  assert.equal(qual.qualified, true);
  assert.equal(a.activate(TELECOMMUNICATIONS_ENGINE_ID, qual.qualified)?.toState, 'ACTIVE');
});

test('IES016-FI-ACC6: telecommunications transport via shared generic DTO', () => {
  const { clock } = makeRuntime();
  const t = new Transport(clock);
  const dto = t.build(TELECOMMUNICATIONS_ENGINE_ID, [
    { sectorId: 'SEC-TC', sectorFamily: 'Telecommunications', companyName: 'TC-001', metrics: { revenueGrowth: 6 }, scores: { composite: 77.8 }, verdict: 'Buy' },
  ]);
  assert.equal(t.validate(dto), true);
  assert.equal(t.serialize(dto), t.serialize(dto));
});

test('IES016-FI-ACC7: 14 plugins coexist through the same framework without branching', () => {
  const { plugins, runtime, store } = makeRuntime();
  ...
  runtime.execute(AUTOMOBILE_ENGINE_ID, { requestId: 't17', inputs: {"subsegment":"mass-market-oem","archetype":"full-line","ebitdaMargin":12,"revenueGrowth":8,"debtEbitda":1.8,"vehicleMargin":9,"capacityUtilization":85,"evMix":30,"fcfYield":5,"roic":12,"capexIntensity":12,"inventoryDays":45,"evEbitda":5.5,"aftersalesMix":18} });
  ...
  const self = runtime.execute(TELECOMMUNICATIONS_ENGINE_ID, { requestId: 'ies016', inputs: {"subsegment":"fixed-broadband","archetype":"enterprise","ebitdaMargin":38,"revenueGrowth":4,"debtEbitda":2.5,"arpu":45,"churnRate":1.4,"postpaidMix":75,"fcfYield":5,"roic":11,"capexIntensity":12,"spectrumCost":0.3,"evEbitda":7,"usageGrowth":14} });

  assert.equal(self.result.state, 'COMPLETED');
  assert.equal(self.result.metadata.composite, 71.4);
