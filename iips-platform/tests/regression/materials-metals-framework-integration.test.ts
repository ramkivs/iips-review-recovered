/**
 * IES-020 — Framework Integration (D28 fence-4 relief, Tier-3 A1 evidence).
 * Proves Materials & Metals integrates through all shared framework services unchanged (manifest,
 ...
 * Inputs are the frozen golden-reference providers MM-001 / MM-002.
 */
...
test('IES020-FI-ACC1: materials-metals manifest via shared ManifestLoader', () => {
  const engine = new MaterialsMetalsEngine();
  const m = new ManifestLoader().load(engine.manifest);
  assert.equal(m.engineId, MATERIALS_METALS_ENGINE_ID);
  assert.equal(Object.isFrozen(m), true);
  assert.ok(m.capabilities.includes('calibration'));
});

test('IES020-FI-ACC2: materials-metals evidence via shared EvidencePipeline', () => {
  const { evidence } = makeRuntime();
  const pkg = evidence.build({
    engineId: MATERIALS_METALS_ENGINE_ID, recommendation: 'Strong Buy', compositeScore: 82.5, confidence: 0.8,
    supportingScores: [{ id: 'reserveLife', name: 'Reserve Life', value: 22 }],
    calibrationVersion: 'materials-metals-calibration-1.0.0', replayReference: 'snap',
    provenance: { frameworkVersion: '1.0', engineVersion: '1.0.0', methodologyVersion: 'IES-020 v1.0', snapshotId: 's1' },
  });
  assert.equal(evidence.validate(pkg), true);
});

test('IES020-FI-ACC3: materials-metals deterministic snapshot via shared Snapshot', () => {
  ...
  const r = runtime.execute(MATERIALS_METALS_ENGINE_ID, { requestId: 'ies020-snap', inputs: {"subsegment":"diversified-miners","archetype":"integrated","ebitdaMargin":32,"revenueGrowth":9,"debtEbitda":1.4,"reserveLife":22,"cashCostCurve":20,"realizedPriceSpread":106,"fcfYield":8,"roic":14,"capexIntensity":11,"inventoryDays":40,"evEbitda":5.5,"recyclingInputMix":15} });
  assert.equal(r.result.state, 'COMPLETED');
  assert.equal(store.size, 1);
  assert.ok(r.result.snapshotRef);
});

test('IES020-FI-ACC4: materials-metals replay via shared ReplayService', () => {
  const { plugins, runtime, replay } = makeRuntime();
  plugins.load(new MaterialsMetalsEngine());
  plugins.initialize(MATERIALS_METALS_ENGINE_ID);
  const snap = runtime.recordSnapshot(MATERIALS_METALS_ENGINE_ID, { revenueGrowth: 9 }, { composite: 82.5 }, 'MATERIALS_METALS_SUMMARY');
  assert.equal(Object.isFrozen(snap), true);
  assert.equal(replay.replay(snap.snapshotId)?.reproduced, true);
});

test('IES020-FI-ACC5: materials-metals diagnostics + qualification + activation via shared framework', () => {
  const d = new DiagnosticsService();
  const q = new QualificationService();
  const a = new ActivationService();
  d.capture({ engineId: MATERIALS_METALS_ENGINE_ID, executionDurationMs: 3, registryVersions: { transport: '1.0' }, replayStatus: 'ok', transportStatus: 'ok', pluginPhase: 'Execution' });
  assert.equal(d.list()[0].engineId, MATERIALS_METALS_ENGINE_ID);
  const qual = q.qualify({ engineId: MATERIALS_METALS_ENGINE_ID, certified: true, replayVerified: true, regressionPassed: true, deterministic: true });
  assert.equal(qual.qualified, true);
  assert.equal(a.activate(MATERIALS_METALS_ENGINE_ID, qual.qualified)?.toState, 'ACTIVE');
});

test('IES020-FI-ACC6: materials-metals transport via shared generic DTO', () => {
  const { clock } = makeRuntime();
  const t = new Transport(clock);
  const dto = t.build(MATERIALS_METALS_ENGINE_ID, [
    { sectorId: 'SEC-MM', sectorFamily: 'Materials & Metals', companyName: 'MM-001', metrics: { revenueGrowth: 9 }, scores: { composite: 82.5 }, verdict: 'Strong Buy' },
  ]);
  assert.equal(t.validate(dto), true);
  assert.equal(t.serialize(dto), t.serialize(dto));
});

test('IES020-FI-ACC7: 14 plugins coexist through the same framework without branching', () => {
  const { plugins, runtime, store } = makeRuntime();
  ...
  const self = runtime.execute(MATERIALS_METALS_ENGINE_ID, { requestId: 'ies020', inputs: {"subsegment":"base-metals","archetype":"pure-play","ebitdaMargin":30,"revenueGrowth":10,"debtEbitda":1.6,"reserveLife":14,"cashCostCurve":30,"realizedPriceSpread":101,"fcfYield":6,"roic":13,"capexIntensity":14,"inventoryDays":45,"evEbitda":6,"recyclingInputMix":10} });

  assert.equal(self.result.state, 'COMPLETED');
  assert.equal(self.result.metadata.composite, 76.6);
