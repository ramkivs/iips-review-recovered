# DEC-G-AI-IMPL-CERT-CRITERIA — Certification Criteria, Live-Validation Policy & Dispatch Coverage
...
- **Class:** `DECISION`
- **Status:** `RECORDED — CRITERIA DEFINED`
- **Date:** 2026-08-27
- **Authority relationship:** gate `G-AI-IMPL CERTIFICATION-PRECONDITION AUTHORITY`, §5, §6
...
gate". `AUTH-G-AI-IMPL` §4 expressly places *"Certification, or any attempt at
...
## 2. DECISION — CERTIFICATION CRITERIA

| ID | Criterion | Classification | Current status |
|---|---|---|---|
| **A** | Static / source validation (S1, S2 exact text, S3-A, S4 codes, SR-1…SR-5 markers, D6/D7) | **REQUIRED** | **PASSED** — 11/11 checks against the pushed blob |
| **B** | Typecheck — client `tsc --noEmit` **and** server `tsc --noEmit -p tsconfig.server.json` | **REQUIRED** | **PASSED** — both exit 0 |
| **C** | Production build (`tsc -b && vite build`) | **REQUIRED** | **PASSED** — exit 0 |
| **D** | Complete automated suite (`vitest run`) | **REQUIRED** | **PASSED** — 651 passed / 21 skipped / 0 failed |
| **E** | Test obligations **T1 … T10** | **REQUIRED** | **PASSED** — 130 passed / 0 failed |
| **F** | Specification requirements **SR-1 … SR-5** | **REQUIRED** | **PASSED** |
| **G** | Real HTTP transport verification | **REQUIRED ONLY IF INFRASTRUCTURE AVAILABLE** | see §3 |
| **H** | Authenticated live HTTP 200 advisory request | **REQUIRED ONLY IF INFRASTRUCTURE AVAILABLE** | **NOT PERFORMED** — no IdP |
| **I** | Real Keycloak authentication | **REQUIRED ONLY IF INFRASTRUCTURE AVAILABLE** | **NOT PERFORMED** — no container runtime, no IdP |
| **J** | Live browser rendering | **REQUIRED ONLY IF INFRASTRUCTURE AVAILABLE** | **NOT PERFORMED** — no browser |
| **K** | Recovery-evidence and ten-fence verification | **REQUIRED** | **PASSED** — 10/10 unchanged `85bbd49`→`e5d59981`; six blobs preserved at `origin/arena` |
...
### Note on criterion G — recorded for accuracy, not to vary the decision
...
repository already contains a working real-socket test pattern —
...
G-DISPATCH-COVERAGE authorization in §4 will in fact provide it. The classification stands
as decided; this note only prevents the classification from being read as "G is impossible
here", which is not true.

## 3. DECISION — G-LIVE-TEST: **OPTION D**
...
## 4. DECISION — G-DISPATCH-COVERAGE

### 4.1 Finding (new, this gate)

The `/api/ai-advisory/` dispatch branch in `frontend/server/executive-transport.ts` — the
only production path that reaches the feature — is exercised by **no** test:
...
- Its `req`/`res` are **stub objects** built by object literal and cast
  (`as unknown as http.IncomingMessage` / `as unknown as http.ServerResponse`) — no socket.
- Its `OidcVerifier` is **mocked** (`{ verify: vi.fn().mockResolvedValue(...) }`) — no real
  authentication.
...
### 4.2 Decision — **OPTION: cover it inside the existing authorized test file**
...
| Constraint | Value |
|---|---|
| New path created | **NONE** — `frontend/server/ai-advisory-transport.test.ts` is already one of the 13 authorized paths |
...
| External infrastructure required | **NONE** — no container runtime, no IdP |
| Implementation behavior | **UNCHANGED** — this is test-only; no source file other than the named test file may be modified |
| Methodology touched | **NONE** — S1–S4, SR-1–SR-5, B1–B4, D1–D8, P7, TRIM-S, TRIM-V, HARVEST, EXIT, NO-DECISION all unchanged |
...
| **D-CERT-CRITERIA** | **RESOLVED** by §2 of this record |
| **G-LIVE-TEST** | **RESOLVED** as Option D — live validation is a recorded limitation, not a blocker |
| **G-DISPATCH-COVERAGE** | **AUTHORIZED, NOT YET PERFORMED** — outstanding work item |
...
addition (§4). Everything else is resolved or is a recorded environment limitation.
...
| Modify any source file | **NO** — the §4 authorization is test-only and is executed at a later gate |
| Mark any unavailable test as PASS | **NO** |
| Import, restore or derive from the recovered live-certification test | **NO** |

## 7. CLASSIFICATION

- **D-CERT-CRITERIA — RESOLVED** (criteria defined, §2)
- **G-LIVE-TEST — RESOLVED as OPTION D** (§3)
- **G-DISPATCH-COVERAGE — AUTHORIZED, OUTSTANDING** (§4)
