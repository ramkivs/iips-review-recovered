# DEC-D5-EVIDENCE-MATURITY — Class-A Evidence Sub-Classification (A1 / A2)

- **Record ID:** `DEC-D5-EVIDENCE-MATURITY`
- **Title:** D5 — Evidence Parity / Class-A Definition: Evidence Maturity Sub-Classification Authorized
- **Class:** `DECISION` / `METHODOLOGY AUTHORITY`
- **Status:** `RECORDED — OPTION A AUTHORIZED`
- **Date:** 2026-08-28
- **Authority relationship:** gate `PROGRAM v3.0 / IES CAPABILITY RE-BASELINE AUTHORITY GATE`,
  ...
  selected **A — define evidence maturity explicitly**. Recording authority (record +
...
— *"IMPLEMENTED + CERTIFIED + RUNTIME INTEGRATED + UI INTEGRATED"* — uniformly to all 14
...
|---|---|
| **A1 — Full evidence** | Independent verification **+** final readiness **+** freeze / provenance evidence **+** required regression evidence |
| **A2 — Partial evidence** | Capability and implementation validated, but **one or more** full-evidence artifacts absent |

**Capability status is NOT changed by this decision.** All 14 capabilities remain Class A —
...
|---|---|---|---|---|---|---|---|
| IES-006.2A | banking | 1 | — | — | 1 | 11 | **A2** |
| IES-007 | insurance | 1 | — | — | 3 | 11 | **A2** |
| IES-008 | capital-markets | 1 | — | — | 3 | 10 | **A2** |
| IES-009 | healthcare | 1 | — | — | 3 | 10 | **A2** |
...
| IES-016 | telecommunications | — | — | — | 2 | 11 | **A2** |
| IES-017 | automobile | — | — | — | 2 | 11 | **A2** |
| IES-020 | materials-metals | — | — | — | 2 | 11 | **A2** |
| CSIP | cross-sector | 1 | 1 | 1 | 4 | 13 | **A1** |

```
A1 — FULL EVIDENCE    (7):  IES-010, IES-011, IES-012, IES-013, IES-014, IES-015, CSIP
A2 — PARTIAL EVIDENCE (7):  IES-006.2A, IES-007, IES-008, IES-009, IES-016, IES-017, IES-020
```

Artifact locations, for auditability: the independent-verification reports and
final-readiness certificates for IES-010…015 and CSIP live in `iips-platform/`
(`IES010…IES015_INDEPENDENT_VERIFICATION_REPORT.md`, `IES010…IES015_FINAL_READINESS_CERTIFICATE.md`,
`CSIP_INDEPENDENT_VERIFICATION_REPORT.md`, `CSIP_FINAL_READINESS_CERTIFICATE.md`); those for
IES-006.2A…009 live in `iips-platform/reports/`, `reports-insurance/`,
`reports-capital-markets/`, `reports-healthcare/`. Freeze manifests live inside the
`ies-0xx-*/` packs (IES-010…015) and `iips-cross-sector/`. **There are no `IES016_*`,
...
The A1 definition's fourth limb — *"required regression evidence"* — has **no quantified
...
| ≥ 3 | 7 | YES |
| ≥ 4 | 7 | YES |
| ≥ 5 | 0 | no (no capability has 5+ regression test files) |
...
files each), and each carries the full frozen-asset set — `calibration/`,
`fixtures/…golden-reference`, `expected-outputs/`, `fixtures/…validation-fixtures`,
`replay-datasets/`, `contract-tests/generate_expected_outputs.py`,
`*-ontology-metadata-1.0.0.json` — plus a discovery pack, an acceptance matrix, an
...
1. an **evidence-maturity column** (or equivalent annotation) on the matrix's per-engine
   table;
2. the **7 A1 / 7 A2 designations** from §3 recorded against the corresponding rows;
...
class A — a known, recorded divergence.
...
### 5.2 D4 — AI Advisory matrix / merge treatment
...
- If D4 authorizes a matrix row for AI Advisory, that row must carry its **own evidence
  maturity designation**. On the recorded evidence it would be **A2-partial**: criteria A–F,
  K, L, M1–M3 PASS, with **H (authenticated live 200), I (real Keycloak) and J (live
  browser) NOT PERFORMED** under the Option-D limitation in
  `DEC-G-AI-IMPL-CERTIFICATION` §5. That is precisely the A2 condition — capability and
...
|---|---|---|---|
| **D5-S1** | The A1 "required regression evidence" limb is **not quantified** | No authority record defines a regression-count or regression-coverage threshold | Quantify it at a later authority decision. **Non-blocking** — §3.2 proves the current 7/7 split is insensitive to any threshold 0–4 |
...
|---|---|
| **B — require the full bundle for Class A** | Would have made independent-verification + final-readiness + freeze evidence mandatory, rendering **7 of 14 capabilities evidence-blocked** — including `banking`, the declared reference baseline, and all of IES-006.2A…009. That changes **capability status on evidence grounds**, which option A explicitly avoids. Closing it would have required authoring 7 independent-verification reports, 7 final-readiness certificates, 7 freeze manifests, and additional regression tests for banking / 016 / 017 / 020 |
| **C — keep Class A evidence-neutral** | Would have left "all 14 class A" concealing a three-tier evidence reality, and left the IES-006.2A…009 and `banking` asymmetries unaddressed — the asymmetry is **not** confined to IES-016/017/020 |
| **D — remain deferred** | No D5 methodology authority; D3, D4, D2 and D6 could not proceed |

## 8. GUARDRAILS HONOURED

| Prohibited | Status |
|---|---|
| Modify the matrix | **NOT DONE** — `docs/v3.0/INTEGRATION_VERIFICATION_MATRIX.md` remains `4967b0232afc5e4044951f03d141f4225e435693` |
| Create or modify evidence artifacts | **NOT DONE** — no independent-verification, final-readiness or freeze artifact was authored |
| Merge AI Advisory | **NOT DONE** — `origin/gai-impl-canonical` unchanged at `f63a9b49` |
| Modify `phase13-next` | **NOT DONE** — unchanged at `85bbd49` |
| Change any engine | **NOT DONE** — `iips-platform/**` untouched |
| Restore pruned packs | **NOT DONE** — `ies-005…009` remain absent per the recorded Option A prune |
| Reopen P7 | **NOT DONE** — and P7 is not claimed as PASS; no P7 artifact exists |
| Alter certification | **NOT DONE** — `DEC-G-AI-IMPL-CERTIFICATION` blob `968ad4bf8e8f…` unchanged |
| Promote a version / release | **NOT DONE** — tag `v3.0-phase12-certified` unchanged |
...
| Item | Status |
|---|---|
| D5 | **RESOLVED — Option A authorized; A1/A2 defined; 14 capabilities re-tiered 7 / 7** |
| Capability status of any engine | **UNCHANGED — all 14 remain Class A** |
| D5-S1 regression threshold | **OPEN, non-blocking** |
| D3 | **OPEN — scope now includes the A1/A2 column; requires matrix-amendment authority** |
| D4 | **OPEN — merge and matrix-row decisions remain separate; neither granted** |
| D2 / D6 | **OPEN — not decided by this gate** |
| Implementation authorized | **NO** |
| P7 | **NOT REOPENED, NOT claimed as PASS** |

## 11. CLASSIFICATION

# **D5 — OPTION A AUTHORIZED**

Class A now carries an explicit evidence-maturity sub-classification. All 14 capabilities
remain Class A. Seven are **A1 (full evidence)** — IES-010, 011, 012, 013, 014, 015 and
CSIP. Seven are **A2 (partial evidence)** — IES-006.2A, 007, 008, 009, 016, 017 and 020.
