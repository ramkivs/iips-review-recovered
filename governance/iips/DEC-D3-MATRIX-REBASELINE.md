# DEC-D3-MATRIX-REBASELINE — D3 Matrix Re-Baseline Decisions

- **Record ID:** `DEC-D3-MATRIX-REBASELINE`
- **Title:** D3 — Matrix Basis Convention, Evidence-Maturity Representation, Regression
  Threshold, AI Advisory Treatment and UI-Surface Column Semantics
- **Class:** `DECISION` / `AUTHORITY`
- **Status:** `RECORDED — D3-1…D3-5 DECIDED. MATRIX AMENDMENT NOT AUTHORIZED.`
- **Date:** 2026-08-28
- **Authority relationship:** gate `PROGRAM v3.0 / IES CAPABILITY RE-BASELINE — D3 MATRIX
...
| Present in the `arena` governance tree? | **NO** — canonical lineage only |
| Stated basis (verbatim, line 4) | *"**Basis:** canonical `phase13-next` @ `7f6b27d5e28ce3ec96b2b8c7fd00faecbd2445aa`"* |

### 1.2 Actual timeline — only two commits have ever touched the matrix
...
| `3514d47908afa560d6c733d815452adae8bd9e91` | 2026-08-20 23:32:20 +0530 | `docs: reconcile standards inventory for IES-016/017/020` | **LAST SUBSTANTIVE UPDATE** (+11/−8) |
| `85bbd49cd31c215a8fd0e7651b718861944dfe45` | 2026-08-25 | `feat: add P-2 notes surface` | canonical tip — matrix untouched |

**The matrix does not exist at its own stated basis:**
`git cat-file -e 7f6b27d5…:docs/v3.0/INTEGRATION_VERIFICATION_MATRIX.md` → *fatal: path
exists on disk, but not in `7f6b27d5…`*. At `7f6b27d5` the strings `IES-016`, `IES-017`,
`IES-020` occur **0** times (the file is absent); at canonical, **2** times each — added by
...
| Measure | Value |
|---|---|
| **Matrix content drift** `3514d47` → `85bbd49` | **ZERO** — blob `4967b0232afc…` identical at both |
| Commits | 13 |
| Files changed, all | 41 |
| Files changed under `iips-platform/` | **0** |
| Files changed under `iips-platform/src/sector-engines/` | **0** |
| Files changed in the engine+transport surface | **1** — `frontend/server/executive-transport.ts` |
| Diff lines in that file touching `ENGINE_FACTORY` / `*_ENGINE_ID` / registration | **0** |

### 1.4 Capability claims all reconcile

14 engine directories · 13 `sector.*` engine IDs · `CrossSectorEngine.ts` present ·
`ENGINE_FACTORY` = 13 entries · `runtime.execute` linkage unchanged · v1.1 replay baseline =
...
threshold for any value 0–4, because every A2 member already fails at least one
non-regression limb.

---

## 2. D3-1 — BASIS CONVENTION → **OPTION B AUTHORIZED**

**Decision:** the matrix basis is the commit at which the substantive reconciliation was
actually performed — **`3514d47908afa560d6c733d815452adae8bd9e91`** — with an **explicit
...
> byte-identical between the two (`4967b0232afc…`); `0` files changed under
...
assert a reconciliation *at* `85bbd49` that has no commit behind it — repeating the original
...
## 3. D3-2 — EVIDENCE-MATURITY REPRESENTATION → **MINIMAL AMENDMENT + UI-SURFACE DISPOSITION**
...
1. **One added column**, `Evidence`, on the existing per-engine table, valued `A1` or `A2`.
   ...
   A–F class key.
2. **The 7 / 7 designations** exactly as recorded in `DEC-D5-EVIDENCE-MATURITY` (§1.5).
...
4. **Evidence-depth provenance**: cite `DEC-D5-EVIDENCE-MATURITY` as the classification
   source, and name the artifact locations relied on — `iips-platform/IES0xx_*`,
   `iips-platform/reports*/`, `ies-0xx-*/IES-0xx_FREEZE_MANIFEST.json`,
   `iips-platform/tests/regression/`.
5. **The D5-S1 note** required by D3-3 (§4).
6. **The UI-surface column semantics and Screener determination** required by §5.

**Explicitly NOT authorized / not required:** re-deriving the tiers in the matrix,
restating the A1 definition, changing any capability class, altering the A–F key, or any
change beyond items 1–6.

### 3.2 Estimated amendment size

One column across 14 rows · two short prose blocks (class-A-unchanged, provenance) · one
D5-S1 note · one UI-semantics sentence · one cell change (§5.2). Nothing else.

## 4. D3-3 — D5-S1 REGRESSION THRESHOLD → **OPTION A AUTHORIZED**
...
leaving it open, because the classification is provably threshold-insensitive across 0–4.
...
## 5. D3-5 — UI-SURFACE COLUMN SEMANTICS (arising from the discovery, dispositioned here)
...
`3514d47` four feature directories appeared — `screener`, `shell`, `notes`, `notifications`.
...
- `Screener.tsx` header: *"Engine-aware, read-only screening surface over the governed
  ...
  (`fetchDecisionMatrixData`) — no new endpoint, no new authority, no persistence, no
  recomputation. Renders governed values 1:1 … a filter-and-navigate surface, NOT an
  analytics engine."*
- It is navigable and live: `frontend/src/app/navigation.ts:73` —
...
- `computeCertifiedDecisionMatrix()` maps over `engineDetails` keyed by `d.sector`,
  ...
  valuation pillar) — i.e. a **composed cross-sector table**, structurally identical to
...
|---|---|
| Macro (`/research/macro`, `navigation.ts:75`) | MoSPI national-statistics source (`server/macro/mospi-source.ts`) — external data, not engine output |
| CommandPalette (`features/shell`) | One client-side filter on a `sector` **string field** (`companies.filter((c) => c.sector.toLowerCase().includes(q))`) — not an engine call |
...
## 6. D3-4 — AI ADVISORY → **DEFERRED ENTIRELY TO D4**
...
> treatment in this matrix — and whether it is merged into `phase13-next` — is decided
> separately under **D4**.
...
evidence maturity would be **A2-partial** (criteria A–F, K, L, M1–M3 PASS; H/I/J NOT
...
|---|---|---|
| **D3-1** | Basis convention | **B** — basis `3514d47908afa560d6c733d815452adae8bd9e91`, with explicit re-verification recorded at `85bbd49cd31c215a8fd0e7651b718861944dfe45`. Convention adopted: "Basis" = the commit reconciled against; a later no-content-change re-verification is recorded alongside, not substituted |
| **D3-2** | Evidence-maturity representation | **Minimal amendment** — one `Evidence` column (A1/A2), the 7/7 designations, an explicit "class A unchanged" statement, provenance citation to `DEC-D5-EVIDENCE-MATURITY`; **plus** the UI-surface disposition in §5 |
| **D3-3** | D5-S1 regression threshold | **A** — remains explicitly unquantified; the amendment must say so and note the classification is threshold-insensitive for 0–4. **D5-S1 stays OPEN** |
| **D3-4** | AI Advisory in D3 | **Deferred entirely to D4** — forward-reference sentence only; no row, no reserved row |
| **D3-5** | UI-surface column semantics | **Decided** — semantics defined; **Screener recorded on the CSIP row** alongside Decision Matrix; the 13 sector rows unchanged; Macro / CommandPalette / Notes / Notifications excluded with evidence |

## 8. WHAT THIS RECORD DOES NOT AUTHORIZE

| Item | Status |
|---|---|
| Modify `docs/v3.0/INTEGRATION_VERIFICATION_MATRIX.md` | **NOT AUTHORIZED** — requires a separate matrix-amendment execution gate |
...
| Restore pruned `ies-005…009` packs | **NOT AUTHORIZED** |
| Reopen P7 | **NOT DONE** — and P7 is not claimed as PASS |
| Alter certification | **NOT AUTHORIZED** |
| Promote a version or release | **NOT AUTHORIZED** |
| Authorize a regression threshold | **NOT DONE** — D5-S1 remains open |
| Decide D5-S3 (A2 → A1) | **NOT DECIDED** |
...
| **D2** | Unaffected. Note for D2: `INTEGRATION_VERIFICATION_MATRIX.md` is a **real, existing** artifact, unlike the "Engine Master Matrix" named in the `governance/iips/` prohibitions — D2 must not conflate the two |
...
**`PROGRAM v3.0 / IES CAPABILITY RE-BASELINE — D3 MATRIX-AMENDMENT EXECUTION GATE`.**

Its authority must be explicit and must cover exactly: applying items 1–6 of §3.1 plus the
§5.2 cell change and the §6 forward-reference sentence to
`docs/v3.0/INTEGRATION_VERIFICATION_MATRIX.md`, on the canonical line, as a
documentation-only commit — with verification that no capability class, A–F key, engine
...
# **D3-1…D3-5 DECIDED — MATRIX AMENDMENT NOT YET AUTHORIZED**
