# DEC-D6-DURABLE-RECORDING-POLICY — D6 Durable Recording Policy

- **Record ID:** `DEC-D6-DURABLE-RECORDING-POLICY`
- **Title:** D6 — Authoritative Durable Recording Location for the Program v3.0 / IES Capability Re-Baseline
- **Class:** `DECISION` / `POLICY`
- **Status:** `RECORDED — D6 = OPTION A`
- **Date:** 2026-08-28
- **Authority relationship:** gate `PROGRAM v3.0 / IES CAPABILITY RE-BASELINE — D6 DURABLE
...
### 2.1 Out-of-repo reports do not survive — 13 of 13 destroyed
...
`PROGRAM-V3-IES-CAPABILITY-REBASELINE-DISCOVERY`, `PROGRAM-V3-IES-D3-DISCOVERY-RECORD` —
...
written within the preceding few turns — i.e. their survival is luck, not policy.
...
|---|---|---|
| `origin/arena/…` | **26 records** | recovery-lineage tree |
...
## 3. KNOWN ACCEPTED CONSEQUENCE — four dangling citations
...
|---|---|
| `governance/iips/DEC-D3-MATRIX-REBASELINE.md` | **NO — DANGLING** |
| `governance/iips/DEC-D5-EVIDENCE-MATURITY.md` | **NO — DANGLING** |
| `governance/iips/DEC-G-AI-IMPL-CERT-CRITERIA.md` | **NO — DANGLING** |
| `governance/iips/DEC-G-AI-IMPL-CERTIFICATION.md` | **NO — DANGLING** |
...
broken references to be repaired by adding files to `phase13-next`** — that would violate
...
|---|---|
| **B — product-branch recording** | Requires commits to `phase13-next`, which holds 0 `governance/iips/` records today; either migrates 26 records (relocating established authority) or duplicates them (two copies of the same authority); mixes gate records into product history; `arena` would cease to be the single store or become a stale copy |
| **C — dual record** | Two commits on two unrelated lineages per decision; divergence risk; "which copy is authoritative" would itself need defining; still requires `phase13-next` commits; highest ongoing cost |
| **D — defer** | The 13-of-13 out-of-repo destruction pattern and the four dangling citations would both persist, and every future gate would re-litigate where its record belongs |
...
| Item | Status |
|---|---|
| Migrate, duplicate or delete any existing record | **NOT DONE** — all 26 pre-existing records byte-identical |
| Modify `phase13-next`, `program-v3-matrix-rebaseline`, `gai-impl-canonical` or `main` | **NOT DONE** |
| Modify the Integration Verification Matrix | **NOT DONE** — still `cada04514004…` |
| Modify `AUTH-G-AI-IMPL` or `SPEC-G-AI-IMPL` | **NOT DONE** |
| Annotate or repair the four dangling citations | **NOT DONE** — requires separate matrix-amendment authority |
| Modify engines or implementation | **NOT DONE** |
| Create evidence artifacts | **NOT DONE** |
| Define the D5-S1 regression threshold | **NOT DONE** — remains explicitly undefined |
| Change D5-S3 | **NOT DONE** — remains Option B |
| Reopen P7 or claim it as PASS | **NOT DONE** |
| Perform H/I/J live validation | **NOT DONE** — remain Option-D limitations |
...
# Next: **D5-S1 — REGRESSION-EVIDENCE THRESHOLD AUTHORITY GATE**
...
A1 closure — but A1's fourth limb, *"required regression evidence"*, is **explicitly
...
the observed 1/2/3/4 regression-test counts** — they are evidence, not a threshold.

**Explicitly not pre-decided here.** This record neither selects (a) nor (b), nor proposes a
number.

**Sequenced after it**, each requiring its own authority: **A2→A1 evidence-debt closure**
(needs D5-S1 for the regression limb; the three non-regression artifacts could close
independently) · **`program-v3-matrix-rebaseline` disposition** (its content is already on
`phase13-next` via `33dc1a7d…`) · **dangling-citation annotation** (per §3) ·
**`AUTH-G-AI-IMPL` §4 annotation** (still names three referent-less artifacts per
`DEC-D2-DANGLING-VOCABULARY`) · **H/I/J closure** (infrastructure, not authority; not
self-clearing) · **P7 referent** (identify, or record permanently that it has no referent;
never claimed as PASS).

## 7. CLASSIFICATION

# **D6 — OPTION A RECORDED**
