# DEC-D2-DANGLING-VOCABULARY — D2 Vocabulary Disposition

- **Record ID:** `DEC-D2-DANGLING-VOCABULARY`
- **Title:** D2 — Disposition of Program Vocabulary Terms With and Without Authoritative Referents
- **Class:** `DECISION` / `AUTHORITY`
- **Status:** `RECORDED — D2 = OPTION A`
- **Date:** 2026-08-28
- **Authority relationship:** gate `PROGRAM v3.0 / IES CAPABILITY RE-BASELINE — D2 + D5-S3
  ...
  selected **A — record only evidenced referents and mark the rest as
...
Several program terms appear in this governance store as **prohibitions** — most prominently
`AUTH-G-AI-IMPL` §4: *"Any change to PC-4, N+5, E2E-017, or the Engine Master Matrix"*. A
...
Exhaustive `git grep` across **all five product refs** —
`origin/main`, `origin/phase13-next` (`357b34dac1bd…`), `origin/gai-impl-canonical`,
`origin/phase13-hardening-delivery`, `origin/program-v3-matrix-rebaseline` — excluding
...
| **`N+3`** | **YES** | `docs/v3.0/phase13-hardening/PHASE13_N3_READ_AUTHORIZATION_CERTIFICATION.md` — an actual certification record certifying the governed READ authorization boundary | **RETAINED AS VALID** (not previously flagged) |
| **`N+*` series** | **YES — extensive** | Present at `phase13-next`: **N+1, N+2, N+3, N+5, N+6, N+7, N+8, N+9, N+10, N+11, N+12, N+13, N+14, N+15, N+16, N+17, N+18, N+20, N+21, N+22**. Absent: **N+4** and **N+19** | **REAL Program v3.0 milestone vocabulary** |
| **`N+4`** | **NO** | 0 occurrences in all five product refs; 1 occurrence, inside this session's own governance record | **INHERITED-WITH-NO-REFERENT** — a gap in an otherwise continuous series |
...
| **`EXIT`** | **NO** (as a program term) | 59 files contain the case-insensitive string, but **all 66 matched tokens are the lowercase word `exit`** — `exit 0` in certification result tables. Filtering `exit N`, `exit code`, `process.exit`, `exited`, `exiting` leaves **zero** standalone occurrences | **FALSE POSITIVE — not program vocabulary** |
| **`P7`** | **NO** | 0 occurrences in tracked non-lockfile files at every product ref; 10 occurrences, all in this session's records | **INHERITED-WITH-NO-REFERENT** — consistent with the standing P7 finding: no artifact, no baseline, never claimed as PASS |
| **`E2E-001` / `E2E-013` / `E2E-014` / `E2E-017` / `E2E-018`** | **NO** | 0 in all five product refs; 1–4 occurrences, all in this session's records | **INHERITED-WITH-NO-REFERENT** — see `DEC-E2E-013-BASELINE` |
| **`Engine Master Matrix`** | **NO** | 0 in all five product refs; 5 occurrences, all in this session's records | **INHERITED-WITH-NO-REFERENT** — the actual capability matrix is `docs/v3.0/INTEGRATION_VERIFICATION_MATRIX.md`, which is a **different artifact and must not be conflated with it** |
| **`Capital Steward`** | **NO** | 0 occurrences **anywhere**, including the governance store | Never entered the record chain; no action |

## 4. STRUCTURAL FINDING — THE TERMS ARE SELF-PROPAGATING
...
**Consequence:** absent a disposition, the chain continues indefinitely — each new record
...
5. **`AUTH-G-AI-IMPL` §4's prohibition is recorded as 3-of-4 unenforceable as written**: of
...
| Amend `AUTH-G-AI-IMPL` or `SPEC-G-AI-IMPL` | **NOT DONE** — requires separate authority |
...
| Reopen P7 or claim it as PASS | **NOT DONE** — no artifact exists |
| Alter certification, release or tag state | **NOT DONE** |
| Resolve D5-S1, D5-S3 or D6 | **NOT DONE** — D5-S3 is decided in `DEC-D5-S3-EVIDENCE-DEBT`; D5-S1 and D6 remain open |

## 7. CLASSIFICATION

# **D2 — OPTION A RECORDED**
