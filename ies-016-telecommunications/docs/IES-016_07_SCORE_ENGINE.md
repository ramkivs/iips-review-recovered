...
# Document 07 - SCORE ENGINE (IES-016 remediated content)
...
2. **Subsegment resolution** (pack §3.4): single `subsegment` -> itself; `subsegments[]` + `subsegmentDominant` in set -> dominant; multi without dominant -> most-conservative risk profile (highest `leverageAlert`, tie-break lexicographic smallest id); archetype `hybrid` -> `hybridDominant` (default `hybrid`). Frozen proof: TC-009 resolves to `wireless-mno`.
3. **Metric scoring** (pack §3.5): each present metric maps through its effective band table to a band score (40/60/75/90 in the baseline tables; calibrated tables may vary per segment).
4. **Effective band-table resolution** (pack §3.5): effective[metric] = calibratedBandTables[subsegment][metric] ?? bandScores[metric], boundaries AND scores together, accepted only at equal band cardinality - otherwise the baseline applies (never mixed). Frozen proof: TC-015 (exact band boundaries).
5. **Pillar composition** (pack §3.6): pillar = renormalized weighted mean of member band scores (weights in document 04/06); missing members are dropped; empty pillar = 0.0.
6. **Composite** (pack §3.7): see document 08. Round-half-to-even at 1 decimal, composite only; pillars stay full precision (presented at 1 decimal in the frozen outputs).
7. **Verdict** (pack §3.8): composite -> six-value verdict via the frozen verdictMapping (document 10).
8. **Overrides** (pack §3.9): min-rank precedence over the base verdict (document 10).

## 2. Reference oracle

The deterministic generator `ies-016-telecommunications/contract-tests/generate_expected_outputs.py` @ `c69ce2eb5d98...` is the recorded reference oracle (a transcription tool, not an authority); the engine must reproduce its outputs exactly - same IEEE-754 arithmetic, same left-to-right summation order. This remediation independently re-transcribed the chain and reproduced all 13 frozen cases and both edge fixtures value-exactly (documents 12/15).

## 3. Frozen structure of a computed case

| Field | Frozen value (TC-001) |
| --- | --- |
| subsegment | wireless-mno |
| declaredSubsegments | ["wireless-mno"] |
| archetype | consumer |
| composite | 77.8 |
| verdict | Buy |
| pillars | {"quality": 80.2, "growth": 75.0, "risk": 75.0, "profitability": 83.2, "capitalEfficiency": 75.0, "valuation": 75.0} |
| overrides | [] |
...
| --- | --- | --- |
| Pipeline contract | ies-016-telecommunications/TELECOMMUNICATIONS_DISCOVERY_PACK.md | 68aae104dd3a0ccad8122d5770bd8d2c61637ba6 |
| Band-table semantics | ies-016-telecommunications/calibration/telecommunications-calibration-1.0.0.json | 178160fcbe0a30975c6796ac22c73a9bd03ab91a |
| Resolution proofs | ies-016-telecommunications/expected-outputs/telecommunications-expected-outputs-1.0.0.json | 0d45ffc44df6d61a6f95dac15a12cb6f88be3155 |
| Reference oracle | ies-016-telecommunications/contract-tests/generate_expected_outputs.py | c69ce2eb5d989f63a0618406b103dc398ebc4948 |
