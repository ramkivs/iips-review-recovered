# IES-016 05 METHODOLOGY PRINCIPLES
...
# Document 05 - METHODOLOGY PRINCIPLES (IES-016 remediated content)
...
- Document type / status header (HISTORICAL): > **Document type:** PROPOSED SPECIFICATION (normative contract D16 v1.0) — awaiting maintainer/domain-authority acceptance. No engine implementation. NOT yet authoritative.
- §0 data authority row (HISTORICAL): > \| Data authority \| **PENDING — maintainer acceptance required.** The frozen reference providers are PROPOSED synthetic fixtures (IES-015 convention), NOT authoritative until the D16 methodology is approved by the maintainer/domain authority \|
- §4 calibration preamble (HISTORICAL): > **Frozen ≠ authoritative.** The calibration values below are proposals awaiting maintainer/domain-authority acceptance. Once accepted they may be frozen (immutable) for certification purposes; until then they carry no authority. File: `calibration/telecommunications-calibration-1.0.0.json` (deep-frozen at load; mirrors TechnologyCalibration).
- Terminal status line (HISTORICAL): > **IES-016 SPECIFICATION STATUS: PROPOSED (AWAITING MAINTAINER ACCEPTANCE)**

## 3. Current authority reconciliation

- Methodology: `D16 v1.0` = ACCEPT (DEC-D25 §1). DEC-D25 §4 additionally verifies the IES-016 calibration "matches pack §4 field-for-field".
...
| --- | --- | --- |
| DEC-D25 acceptance (2026-08-29) | blob cbab4da9ce922aacf45e513954d6e325bb037810 | methodology D16 v1.0 = ACCEPT (fresh, forward-looking; A2 preserved) |
| FROZEN manifest | FROZEN, 2026-08-29, methodologyVersion "IES-016 v1.0 (D16 normative contract)", calibrationProfile telecommunications-calibration-1.0.0 | certification data frozen; postFreezeRule binds |
| Acceptance matrix (16 gates PASS) | ies-016-telecommunications/TELECOMMUNICATIONS_ENGINE_ACCEPTANCE_MATRIX.md | implementation conformance recorded |
| Readiness certificate (recorded) | ies-016-telecommunications/IES-016_IMPLEMENTATION_READINESS_CERTIFICATE.md | evidence-report record incl. the recorded confidence decision |
| Historical pack self-label being reconciled | ies-016-telecommunications/TELECOMMUNICATIONS_DISCOVERY_PACK.md | quoted in §2 above; superseded for authority purposes, retained as history |

## 4. Methodology principles (as accepted; unchanged by this remediation - W8/W9)

1. **Determinism.** No random, clock, or external data at execution time; identical input -> identical output/evidence/metadata (pack §3.7).
2. **Band semantics.** Metric -> band -> score; bands are lower-inclusive / upper-exclusive, terminal band includes its upper bound (pack §3.5).
3. **Effective band-table resolution.** effective table = calibrated ?? baseline, boundaries AND scores together, only at equal band cardinality (pack §3.5).
4. **Missing-data honesty.** A missing primitive is dropped and the pillar renormalizes over available metrics; an empty pillar is 0.0; nothing is fabricated, never NaN (pack §3.1 (TC-001..TC-012); proven by TC-014).
5. **Pillar composition and composite.** Weighted pillars per pack §3.6; composite = round-half-to-even at 1 decimal (composite only), left-to-right IEEE-754 summation in the fixed pillar order (pack §3.7; raw 55.05 rounds to 55.0 (ties-to-even)).
6. **Verdict mapping and overrides.** Six-value verdict vocabulary (pack §3.8); overrides apply deterministic min-rank precedence, governance -> Avoid, all other caps -> Watch; leverage-breach fires automatically at `debtEbitda >= leverageAlert` (pack §3.9).
...
| Pack self-labels (historical wording) | ies-016-telecommunications/TELECOMMUNICATIONS_DISCOVERY_PACK.md | 68aae104dd3a0ccad8122d5770bd8d2c61637ba6 |
| Freeze record | ies-016-telecommunications/IES-016_FREEZE_MANIFEST.json | 70018bdb38849d50af7258f62d0341ac2bf64f1a |
| Normative contract sections | ies-016-telecommunications/TELECOMMUNICATIONS_DISCOVERY_PACK.md | 68aae104dd3a0ccad8122d5770bd8d2c61637ba6 |
