"""Generate IES-016 Telecommunications expected outputs from the D16 v1.0 contract.

Implements the EXACT IES-016 D16 v1.0 normative calculation contract (mirroring the
certified IES-015 D15 v1.3 oracle mechanics):
  ...
  * missing primitive -> dropped + weights renormalized
  * min_rank(baseVerdict, all applicable override caps); leverage-breach when
    debtEbitda >= subsegment leverageAlert
This generator is the reference oracle; the engine implementation must reproduce its
outputs exactly. NO random/date/time sources. Deterministic only.
"""
import json, math, os

BASE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
CAL = json.load(open(os.path.join(BASE, 'calibration', 'telecommunications-calibration-1.0.0.json')))
...
# verdict bands sorted descending (lower-inclusive / upper-exclusive; 100 terminal)
VERDICT = sorted(
...
# pillar composition (D16 §7): quality / growth / risk / profitability / capitalEfficiency / valuation
PILLARS = {
...
def composite(m, subsegment, archetype):
    ...
    return r1h2e(sum(p[k] * w[i] for i, k in enumerate(order)))
...
def final(m):
    ...
    for ovr_field, ovr_id in (
        ('governance', 'governance'), ('regulatoryRisk', 'regulatory-risk'),
        ('competitionPressure', 'competition-pressure'),
        ('subscriberCollapse', 'subscriber-collapse'),
...
def main():
    ref = json.load(open(os.path.join(BASE, 'fixtures', 'telecommunications-golden-reference-1.0.0.json')))
    ...
    with open(os.path.join(out_dir, 'telecommunications-expected-outputs-1.0.0.json'), 'w') as f:
        json.dump({
            'basis': ref['dataset'],
            'standard': 'IES-016',
            'version': '1.0.0',
    ...
    with open(os.path.join(rd_dir, 'telecommunications-replay-dataset-1.0.0.json'), 'w') as f:
        json.dump({
            'dataset': 'telecommunications-replay-dataset-1.0.0',
            'standard': 'IES-016',
            'version': '1.0.0',
            'contractVersion': 'IES-016 v1.0 (D16 normative)',
            'sectors': replay,
