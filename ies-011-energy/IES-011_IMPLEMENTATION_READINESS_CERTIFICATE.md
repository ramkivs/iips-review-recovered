# IES-011 — Implementation Readiness Certificate (ISSUED)
...
| 3 | IES-011 reference assets frozen | ✅ |
| 4 | IES-011 architecture review passed (8/8) | ✅ |
| 5 | Energy implementation authorized against frozen baseline | ✅ |
...
Reuses `iips-platform` unchanged; implements Energy as `sector.energy` (metrics/scoring/calibration/decision/evidence). Registers ontology metadata for CSIP (zero CSIP change). Per Implementation Assumption Register.

## 4. Next authorized phase

**IES-011 Implementation Plan** → WP-1 → WP-2 → WP-3 → WP-4 → Independent Verification → Release.
