...
# Document 07 - SCORE ENGINE (IES-017 remediated content)
...
2. **Subsegment resolution** (pack §4 resolution rules): single `subsegment` -> itself; `subsegments[]` + `subsegmentDominant` in set -> dominant; multi without dominant -> most-conservative risk profile (highest `leverageAlert`, tie-break lexicographic smallest id); archetype `hybrid` -> `hybridDominant` (default `hybrid`). Frozen proof: AB-009 resolves to `premium-oem`.
3. **Metric scoring** (pack §5 (M10)): each present metric maps through its effective band table to a band score (40/60/75/90 in the baseline tables; calibrated tables may vary per segment).
4. **Effective band-table resolution** (pack §5 (M10)): effective[metric] = calibratedBandTables[subsegment][metric] ?? bandScores[metric], boundaries AND scores together, accepted only at equal band cardinality - otherwise the baseline applies (never mixed). Frozen proof: AB-015 (calibrated band boundary, ev-native).
5. **Pillar composition** (pack §5 (M6/M7)): pillar = renormalized weighted mean of member band scores (weights in document 04/06); missing members are dropped; empty pillar = 0.0.
6. **Composite** (pack §5 (M11/M15)): see document 08. Round-half-to-even at 1 decimal, composite only; pillars stay full precision (presented at 1 decimal in the frozen outputs).
7. **Verdict** (pack §5 (M12)): composite -> six-value verdict via the frozen verdictMapping (document 10).
8. **Overrides** (pack §5 (M13/M14)): min-rank precedence over the base verdict (document 10).

## 2. Reference oracle

The deterministic generator `ies-017-automobile/contract-tests/generate_expected_outputs.py` @ `ec599ce1aafb...` is the recorded reference oracle (a transcription tool, not an authority); the engine must reproduce its outputs exactly - same IEEE-754 arithmetic, same left-to-right summation order. This remediation independently re-transcribed the chain and reproduced all 13 frozen cases and both edge fixtures value-exactly (documents 12/15).

## 3. Frozen structure of a computed case

| Field | Frozen value (AB-001) |
| --- | --- |
| subsegment | mass-market-oem |
| declaredSubsegments | ["mass-market-oem"] |
| archetype | full-line |
| composite | 71.3 |
| verdict | Buy |
| pillars | {"quality": 65.2, "growth": 75.0, "risk": 75.0, "profitability": 66.8, "capitalEfficiency": 75.0, "valuation": 75.0} |
| overrides | [] |
...
| --- | --- | --- |
| Pipeline contract | ies-017-automobile/AUTOMOBILE_DISCOVERY_PACK.md | e0ad759f4be4231b18959ae6f22aaa3ec6e2ab0b |
| Band-table semantics | ies-017-automobile/calibration/automobile-calibration-1.0.0.json | e3f84ede6f5e89580aa451a689c0b5689cf8674e |
| Resolution proofs | ies-017-automobile/expected-outputs/automobile-expected-outputs-1.0.0.json | b9982d744d92d592714dcc5b1e8599bed63752f2 |
| Reference oracle | ies-017-automobile/contract-tests/generate_expected_outputs.py | ec599ce1aafb26fe645f238e1f953521e60795f8 |
